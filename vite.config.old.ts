...
interface Workbox {
  precaching: {
    precacheAndRoute: (entries: any[]) => void;
  };
  routing: {
    registerRoute: (route: any, handler: any, method?: string) => void;
  };
  strategies: {
    CacheFirst: any;
...
export default defineConfig(({ mode }) => {
  ...
  const pwaOptions: Partial<VitePWAOptions> = {
    ...
    srcDir: 'public',
    filename: 'sw.js',
    // Configure the service worker scope and registration options
    injectManifest: {
  ...
  return {
    ...
    // Add the PWA plugin with our configuration
    plugins: [
      react({
